...
if(isset($login))
{
 if(trim($uname=="")) { $msg = "Enter the Username"; }
	...
	else
	{
		if($uname=="admin")
		{
		$safe_variable = mysql_real_escape_string("admin");
			if($safe_variable=="admin")
			{
			
			$qry = "select * from admin where username='$uname' && password='".$pwd."'";
			$exe=mysql_query($qry);
			...
			print "alert('SQL INJECTION WARNING ACCESS DENIED')";
			print "</script>";  
			}
		}
		else
		{
			$safe_variable = mysql_real_escape_string($uname);
			if($safe_variable==$uname)
			{
			$qry = "select * from register where username='$uname' && password='".$pwd."'";
			$exe=mysql_query($qry);
			$row=mysql_fetch_array($exe);
			$num=mysql_num_rows($exe);
					if($num==1)
					{
				$pur_qry=mysql_query("select max(pid) from user_purchase");
